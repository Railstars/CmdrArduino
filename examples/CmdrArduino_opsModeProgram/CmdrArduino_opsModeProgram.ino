...
void loop() {
  ...
  if(button_state && (button_state != prev_state))
  {
    ...
    toggle = !toggle;
    if(toggle)
      dps.opsProgramCV(3,DCC_SHORT_ADDRESS, 3,10); //set decoder address 03's CV3 (acceleration rate) to 10
    else
      dps.opsProgramCV(3,DCC_SHORT_ADDRESS, 3,0); //set decoder address 03's CV3 (acceleration rate) to 0
  }
